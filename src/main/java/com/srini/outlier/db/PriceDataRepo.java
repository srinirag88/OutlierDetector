...
import com.srini.outlier.db.tables.Pricedata;
import com.srini.outlier.domian.PriceData;
import com.srini.outlier.persistance.HikariCPDataSource;
...
public class PriceDataRepo {
    ...
    public List<PriceData> getPriceDataFrom(int days) {
        final DSLContext dslContext = DSL.using(HikariCPDataSource.getInstance(),
                SQLDialect.POSTGRES);
        final var data = dslContext.selectFrom(Pricedata.PRICEDATA)
                .where(Pricedata.PRICEDATA.PRICE_DATE.between(OffsetDateTime.now().minusDays(days),
                        OffsetDateTime.now()))
